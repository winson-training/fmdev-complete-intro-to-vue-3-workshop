...
<template>
  <h2>Statistics</h2>
  <ul>
    <li v-for="(stat, type) in benderStatistics" :key="`bender-${stat}-${type}`">{{ type }}: {{ stat }}</li>
  </ul>
  <h2>Characters</h2>
  ...
  <p>
    <span v-for="(character, index) in characterList" :key="`comma-list-character-${index}`">{{ character.name }}{{ index === characterList.length - 1 ? '' :
      ', '
      }}
    </span>
  </p>
